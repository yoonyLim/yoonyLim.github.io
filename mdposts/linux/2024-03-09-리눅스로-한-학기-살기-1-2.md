...
22. 생성한 유저가 슈퍼 유저의 권한을 쓸 수 있도록 설정한다. "EDITOR=vim visudo"(텍스트 에디터 패키지를 nano로 깔았으면 "vim" 대신 "nano"로 입력하면 된다.) 명령어로 파일을 수정하여 "#%wheel ALL=(ALL:ALL) NOPASSWD: ALL" 부분의 "#"을 지워 언코멘트한다. 이제 "su - [사용자 이름]" 명령어로 사용자를 슈퍼 유저로 전환할 수 있다. 
...
24. 호스트 이름을 설정한다. "vim /etc/hostname"으로 파일을 열고 파일에 "[호스트 이름]"을 설정한다. Debian GNU/Linux 셋업 때와 같이 네트워크상 보일 이름이다. 본인은 "archlinux"로 설정했다. 호스트 테이블 또한 수정해야 하므로 "vim /etc/hsots"로 파일을 열고 아래 우측 하단과 같이 작성하고 저장한다. 우측 하단의 "archlinux" 부분은 본인이 지정한 호스트 이름이 들어가면 된다.
...
25. 부팅 시 운영체제 인식을 위해 grub(GRand Unified Boot loader) 및 기타 패키지를 설치한다. "pacman -S grub efibootmgr dosfstools mtools" 명령어로 패키지 설치 후 "grub-install --target=x86_64-efi --efi-directory=/boot --bootloader-id="Arch Linux" --recheck" 입력 후 "grub-mkconfig -o /boot/grub/grub.cfg" 명령어로 설정을 생성하여 부팅 시 BIOS가 OS를 인식하도록 한다.(bootloader-id에 따라 부팅 시 보이는 이름을 설정한다.)
***주의: 일부 BIOS에 따라 부팅 시 GRUB을 인식하지 않는 경우가 있다. 이때 본인의 경우 추후 "--recheck" 플래그를 추가했을 때 정상적으로 인식되었다.***
...
27. 이제 기초 셋업이 끝났다. 이후 자잘하게 블루투스를 원한다면 bluez bluez bluez-utils 패키지 설치 후 "systemctl enable bluetooth", 네트워크 검색을 위해 "systemctl enable NetworkManager"로 설치하도록 하자. 마지막으로 chroot을 "exit" 명령어로 나가고 mount한 디렉토리를 "umoung -lR /mnt"로 unmount한 다음 컴퓨터를 재부팅하면 드라이브에 셋업 완료된 Arch Linux 사용이 가능하다. BIOS에서 Arch Linux가 먼저 부팅되도록 순서 변경하면 이후 재부팅 때마다 grub을 통해 우선적으로 arch linux로 부팅도 가능하다.
