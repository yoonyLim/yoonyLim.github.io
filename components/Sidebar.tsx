    ...
    for (var subject of subjects) {
        if (subject != "projects") {
            ...
            subjectTags.push(
                <Link 
                    href={`/posts/${subject}`}
                    className="flex items-center mt-2 mr-2 py-1 px-4 rounded-full bg-gray-300 dark:bg-gray-500 hover:bg-gray-500 hover:dark:bg-gray-700 transition duration-200 ease-in-out"
                >
    ...
    return (
        <div className="shrink-0 hidden xl:flex flex-col max-w-80">
            ...
            <div className="mt-8 flex flex-col text-gray-500">
                ...
                <span className="mt-8">총 { numPosts }개의 포스트</span>
            </div>
        </div>
    );
}