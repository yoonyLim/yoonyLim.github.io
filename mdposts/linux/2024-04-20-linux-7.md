...
```bash
ffmpeg -i [입력 영상] -acodec copy -f segment -segment_time [자르는 세그먼트 당 시간] \
-vcodec copy -reset_timestamps 1 -map 0 [출력 영상]_%d.[영상 형식]
```
...
```bash
ffmpeg -i [입력 영상 1] -i [입력 영상 2] -filter_complex \
"[1]scale=iw/5:ih/5 [pip]; [0][pip] overlay=main_w-overlay_w-10:10" \
-profile:v main -level 3.1 -b:v 440k -ar 44100 -ab 128k -s 720x400 -vcodec h264 -acodec copy [출력 영상]
```
...
```bash
ffmpeg -i [입력 영상 1] -i [입력 영상 2] -filter_complex \
xfade=transition=[트랜지션 종류]:duration=[트랜지션 길이]:offset=[트랜지션 시작 시간] [출력 영상]
```
...
```bash
ffmpeg -i sample1.mp4 -i sample2.mp4 -filter_complex \
"[1]scale=iw/5:ih/5 [pip]; [0][pip] overlay=main_w-overlay_w-10:10" \
-profile:v main -level 3.1 -b:v 440k -ar 44100 -ab 128k -vcodec h264 -acodec copy -t 10 pip_output.mp4
```
...
3. Transition (fade 효과)

아래는 sample1.mp4를 이용하여 트랜지션을 넣어 20초짜리 영상을 만든 명령어이다. 해당 xfade 라이브러리가 영상 생성 시 포캣이 다른 영상 재생 소프트웨어에 맞지 않는 경우가 생겨 포맷을 yuv420p로 지정하는 것이 좋다.

```bash
ffmpeg -i sample1.mp4 -t 10 -i sample1.mp4 -filter_complex \
xfade=transition=fade:duration=2:offset=8,format=yuv420p -t 20 transition_output.mp4
```

<video autoplay loop controls>
  <source src="https://github.com/yoonyLim/yoonyLim.github.io/assets/64838255/f28117a2-b19c-4b51-99b6-19ffd05091a6" type="video/mp4">
</video>
...
<a id="ref3"></a>
3. Cloudinary, ["FFmpeg"](https://cloudinary.com/glossary/ffmpeg), 2023년 5월 25일 마지막 수정.
<a id="ref4"></a>
4. FFmpeg, ["Xfade"](https://trac.ffmpeg.org/wiki/Xfade), 2023년 9월 마지막 수정.
